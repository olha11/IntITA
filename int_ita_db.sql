...
--
-- Хост: localhost
-- Час створення: Бер 12 2015 р., 15:23
-- Версія сервера: 5.6.14
-- Версія PHP: 5.5.6
...
--
-- База даних: `int_ita_db`
--

-- --------------------------------------------------------

--
-- Структура таблиці `aa_access`
--
...
--
-- Структура таблиці `aa_authorizations`
--
...
--
-- Дамп даних таблиці `aa_authorizations`
--
...
(27, 2, '2015-03-11 11:19:19', '127.0.0.1'),
(28, 2, '2015-03-11 11:19:19', '127.0.0.1');

-- --------------------------------------------------------

--
-- Структура таблиці `aa_errors`
--
...
--
-- Структура таблиці `aa_interfaces`
--
...
(5, 'RU', '/css/images/line2.png', 'image2.png', 'Обучение будущего сегодня', '<p>Программирование - это не так сложно, как ты думаешь. Безусловно, чтобы стать хорошим программистом, нужны время и усилия.</p>', '/index.php?r=site/aboutdetail&id=2', '/css/images/', '', '', '', '', ''),
...
(0, 'UA', '/css/images/Logo_big.png', '/css/images/Logo_small.png', 'Курси', '/index.php?r=courses', 'Викладачі', '/index.php?r=teachers', 'Форум', 'http://www.google.com', 'Про нас', '/index.php?r=site/aboutdetail', 'Вхід'),
(1, 'RU', '/css/images/Logo_big.png', '/css/images/Logo_small.png', 'Курсы', '/index.php?r=courses', 'Преподаватели', '/index.php?r=teachers', 'Форум', 'http://www.google.com', 'О нас', '/index.php?r=site/aboutdetail', 'Вход');
...
(0, 'UA', 'INTITA - Академія програмування', 'ПРОГРАМУЙ  МАЙБУТНЄ', 'Не упусти свій шанс змінити світ - отримай якісну та сучасну освіту <br/>і стань класним спеціалістом! \n\n', '/css/images/slider_img/texture.png', '/css/images/slider_img/line.png', 'ПОЧАТИ', 'Про нас', '/css/images/line1.png', 'Важлива інформація про навчання разом з нами', '1', 'Як розпочати навчання?', 'П’ять кроків до здійснення твоїх мрій', '1', '958px', 'детальніше ...', '/css/images/hexagon.png', 'Готові розпочати?', 'Введіть дані в форму нижче', 'розширена реєстрація', 'Розпочати', 'Ви можете також зареєструватися через соцмережі:', '/css/images/networking.png', '/css/images/formFon.png'),
...
--
-- Дамп даних таблиці `regextended`
--
...
(3, 'UA', 'крок', 3, 'Проплата за навчання', '/css/images/', 'step3.jpg', 'Щоб розпочати проходження курсу чи модуля вибери схему оплати (вся сума за курс, помісячно, потриместрово тощо) та здійсни оплату зручним Тобі способом (схему оплати курсу чи модуля можна змінювати, також можлива помісячна оплата в кредит).'),
...
--
-- Структура таблиці `studentprofile`
--
...
--
-- Дамп даних таблиці `studentprofile`
--
...
--
-- Структура таблиці `studentsaccess`
--
...
--
-- Структура таблиці `teacher_temp`
--
...
--
-- Дамп даних таблиці `teacher_temp`
--
...
--
-- Структура таблиці `theoreticalsmaterials`
--
...
--
-- Дамп даних таблиці `theoreticalsmaterials`
--
...
--
-- Обмеження зовнішнього ключа таблиці `aa_access`
--
...
--
-- Обмеження зовнішнього ключа таблиці `aa_authorizations`
--
...
--
-- Обмеження зовнішнього ключа таблиці `aa_errors`
--
...
--
-- Обмеження зовнішнього ключа таблиці `aa_interfaces`
--
...
--
-- Обмеження зовнішнього ключа таблиці `aa_logs`
--
ALTER TABLE `aa_logs`
...
ALTER TABLE `students`
  ADD CONSTRAINT `FK_students_users` FOREIGN KEY (`email`) REFERENCES `users` (`user_email`);

--
-- Обмеження зовнішнього ключа таблиці `studentsaccess`
--
ALTER TABLE `studentsaccess`
  ADD CONSTRAINT `FK_courseaccess_students` FOREIGN KEY (`studentID`) REFERENCES `students` (`student_id`),
  ...
  ADD CONSTRAINT `FK_studentsaccess_modules` FOREIGN KEY (`moduleID`) REFERENCES `modules` (`module_ID`);
