...
-- Dumping data for table int_ita_db.aa_authorizations: ~20 rows (approximately)
...
-- Dumping data for table int_ita_db.language: ~1 rows (approximately)
...
INSERT INTO `step` (`stepID`, `language`, `stepName`, `stepNumber`, `stepTitle`, `stepImagePath`, `stepImage`, `stepText`) VALUES
	(1, 'UA', 'крок', 1, 'Реєстрація на сайті', '/css/images/', 'step1_4.jpg', 'Щоб Ви отримали доступ до переліку курсів та пройти пробні безкоштовні модулі і заняття зареєструйтесь на сайті.'),
...
CREATE TABLE IF NOT EXISTS `teachers` (
  ...
  CONSTRAINT `FK_teachers_users` FOREIGN KEY (`email`) REFERENCES `users` (`userEmail`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
-- Dumping data for table int_ita_db.users: ~2 rows (approximately)
